...
## 15. Appendix A — Technical Q&A Reference

### A.1 Foundational Concepts

**Q: What is an API?**
An API (Application Programming Interface) is a set of rules that allows two pieces of software to communicate. In SellIt, the React frontend sends HTTP requests to the Django backend's REST API, and the backend responds with JSON data. For example, `GET /api/listings/` returns a list of all active listings as JSON.

**Q: What is a REST API?**
REST (Representational State Transfer) is an architectural style for building APIs using standard HTTP methods:
- `GET /api/listings/` — retrieve listings (Read)
- `POST /api/listings/` — create a listing (Create)
- `PATCH /api/listings/5/` — update listing 5 (Update)
- `DELETE /api/listings/5/` — delete listing 5 (Delete)

Each URL represents a resource, and the HTTP method determines the action.

**Q: What is HTTP and what are HTTP methods?**
HTTP (HyperText Transfer Protocol) is the protocol browsers use to communicate with servers. Main methods: GET (retrieve), POST (create), PATCH/PUT (update), DELETE (remove). Every frontend action in SellIt maps to an HTTP request to our Django API. Status codes like 200 (OK), 201 (Created), 400 (Bad Request), 401 (Unauthorized), 403 (Forbidden), 404 (Not Found) tell the client what happened.

**Q: What is JSON?**
JSON (JavaScript Object Notation) is a lightweight data format for exchanging information. When our React frontend requests a listing, Django returns: `{"id": 1, "title": "iPhone", "price": "500.00"}`. Both JavaScript and Python can parse JSON natively, making it the standard for REST APIs.

**Q: What is CORS and why is it needed?**
CORS (Cross-Origin Resource Sharing) is a browser security mechanism. Our frontend runs on `sell-it-kohl.vercel.app` and backend on `sellit-ma71.onrender.com` — different origins. Browsers block cross-origin requests by default. We use `django-cors-headers` to whitelist our frontend URL in `CORS_ALLOWED_ORIGINS`, telling the browser to allow these specific requests.

**Q: What is a database and why PostgreSQL?**
A database stores structured data in tables. PostgreSQL is a relational database that handles concurrent writes (multiple users buying simultaneously), supports row-level locking (`select_for_update`), and scales for production. It is hosted on Render as a managed service.

**Q: What is SQL?**
SQL (Structured Query Language) is the language for interacting with relational databases. We use Django ORM instead of raw SQL: `Listing.objects.filter(status='active')` generates `SELECT * FROM listings WHERE status='active'` automatically.

**Q: What is an ORM?**
ORM (Object-Relational Mapping) maps database tables to Python classes. Each model (Listing, Order, User) represents a table. Instead of writing SQL, we write Python. The ORM handles query generation and parameterization (preventing SQL injection).

**Q: What is middleware?**
Middleware is code that runs on every request/response cycle before the view processes the request. Our middleware stack: SecurityMiddleware (security headers), WhiteNoiseMiddleware (static files), CorsMiddleware (CORS headers), AuthenticationMiddleware (attaches current user). It is like a pipeline — every request passes through each layer.

**Q: What is a CDN?**
CDN (Content Delivery Network) distributes content across servers worldwide. Cloudinary is our CDN for images — uploads are stored and served from edge servers globally, so a user in any city loads images from the nearest server.

**Q: What is SPA (Single Page Application)?**
In a traditional website, every click loads a new HTML page. In our SPA, the browser loads one `index.html` on first visit. React handles all navigation by swapping components — no full page reloads. React Router changes the component, Axios fetches data, and the page updates instantly.

**Q: What is a component in React?**
A component is a reusable piece of UI defined as a JavaScript function. `ListingCard` shows one listing's image, title, and price. Components accept data via props, can have their own state, and compose together to build the full UI.

**Q: What is state in React?**
State is data that can change over time and triggers UI re-renders. `const [orders, setOrders] = useState([])` creates a state variable. When `setOrders` is called with new data, React re-renders to show updated orders.

**Q: What is JWT?**
JWT (JSON Web Token) is a compact signed token for authentication. Contains `{user_id, role, username, exp}` signed with `SECRET_KEY`. The frontend sends it with every request in the `Authorization: Bearer <token>` header. The server verifies the signature without database queries — stateless authentication.

**Q: What is the difference between authentication and authorization?**
**Authentication** = "Who are you?" — verifying identity via credentials and JWT.
**Authorization** = "What can you do?" — checking role-based permissions. A buyer cannot create listings (403). An admin cannot purchase items (403).

**Q: What is hashing?**
Hashing is a one-way function converting passwords into fixed-length strings. Django uses PBKDF2+SHA256. Even if the database is breached, the original password cannot be recovered from the hash.

**Q: What is an environment variable?**
A key-value pair set outside source code, in the deployment platform or `.env` file. Used for `SECRET_KEY`, `STRIPE_SECRET_KEY`, `DATABASE_URL`. Keeps secrets out of the repository. The `.env` file is in `.gitignore`.

**Q: What is version control (Git)?**
Git tracks every code change as commits with messages, authors, and timestamps. Multiple team members work on different features using branches. GitHub hosts the repository and connects to Render/Vercel for auto-deployment.

**Q: What is deployment?**
Making the application available on the internet. Backend deployed to Render (Gunicorn + PostgreSQL), frontend to Vercel (static files via CDN). Both auto-deploy from GitHub on every push.

**Q: What is a webhook?**
A URL that an external service calls when an event happens. Stripe calls `/api/payments/webhook/` when a payment succeeds. Instead of polling, Stripe proactively notifies us. We verify the request using a cryptographic signature.

**Q: What is escrow?**
A financial arrangement where a third party holds funds until conditions are met. In SellIt, buyer pays -> money held -> seller ships -> buyer confirms delivery -> 7-day protection period -> funds released to seller. If buyer opens dispute, admin decides the outcome.

**Q: What is rate limiting?**
Restricting how many requests a user can make in a time period. Login: 5/min to prevent brute force. Listing creation: 10/hr to prevent spam. Exceeded limits return 429 Too Many Requests.

**Q: What is CSRF?**
CSRF (Cross-Site Request Forgery) is an attack where a malicious website tricks a browser into making unwanted requests. Our API uses JWT tokens in headers (not cookies), so CSRF does not apply. Stripe webhook uses `@csrf_exempt` with signature verification instead.

**Q: What is XSS?**
XSS (Cross-Site Scripting) injects malicious scripts into web pages. React auto-escapes all rendered content. We never use `dangerouslySetInnerHTML`. Backend uses parameterized queries via ORM.

**Q: What is SQL injection?**
Inserting malicious SQL into queries. Django ORM prevents this with parameterized queries — user input is never concatenated into SQL strings.

### A.2 Project-Specific Questions

**Q: Why separate frontend and backend?**
Separation of concerns. React handles UI, Django handles data and business logic. Benefits: independent development, independent deployment, same API can serve a mobile app, each scales independently.

**Q: Why React?**
Component-based architecture with reusable UI pieces. SPA provides instant navigation without page reloads. State management via Context API. Large ecosystem for payment (Stripe.js), charts (Recharts), routing (React Router).

**Q: Why Django?**
Batteries included — ORM, migrations, auth, admin panel, middleware, security. DRF adds serializers, API views, permissions, throttling. Focus on business logic instead of infrastructure.

**Q: Why Vite over Webpack?**
Faster development (instant HMR via native ES modules). Optimized production builds via Rollup. Create React App is deprecated.

**Q: How does the token refresh work?**
Access token expires -> 401 response -> Axios interceptor sends refresh token to `/token/refresh/` -> receives new access token -> stores it -> retries original request. All transparent to the user.

**Q: How does the payment form stay secure?**
Stripe's `<CardElement>` renders a secure iframe controlled by Stripe. Card numbers go directly from browser to Stripe's servers. Our server only handles PaymentIntent IDs. This is PCI compliance — we never handle sensitive card data.

**Q: What is select_for_update()?**
Database row lock. When two buyers click Buy Now simultaneously, `select_for_update()` ensures only one transaction proceeds. The second buyer waits for the lock, then sees the item is already sold.

**Q: What is transaction.atomic()?**
Wraps multiple database operations in one transaction. If any operation fails, all changes roll back. Prevents inconsistent states like a sold listing with no order.

**Q: How does content moderation work?**
`check_listing()` runs on every create/edit. Checks banned keywords and suspicious pricing. Flagged listings appear in admin Support Panel. Report auto-escalation hides listings at 3+ reports.

**Q: How does 2FA work?**
Setup: generate TOTP secret -> QR code -> user scans with authenticator app. Login: after password check, server requires 6-digit TOTP code. Verified with `pyotp.TOTP(secret).verify(code)`. 30-second windows with +/-1 tolerance.

**Q: Why Brevo HTTP API instead of SMTP?**
Render blocks SMTP ports (25, 587). Brevo HTTP API works over port 443 (HTTPS) which is always open.

**Q: Why Cloudinary instead of local storage?**
Render uses ephemeral storage — files deleted on every deploy. Cloudinary stores permanently and serves via global CDN.

**Q: How is admin different from seller and buyer?**
Admin is a pure moderator. Cannot buy, sell, bid, or make offers. Backend returns 403 Forbidden. Frontend hides all buy/sell UI. Admin only accesses Support Panel, Browse (for moderation), and Inbox.

**Q: What is the N+1 query problem?**
20 orders each accessing `order.buyer.username` = 1 query for orders + 20 for buyers = 21 queries. Fixed with `select_related('buyer')` which does a SQL JOIN = 1 query. `prefetch_related` for many-to-many.

**Q: What is Gunicorn?**
Production WSGI server. Django's `runserver` is single-threaded (development only). Gunicorn spawns multiple workers for concurrent requests.

**Q: What is WhiteNoise?**
Middleware serving static files from Django with caching and gzip. Eliminates the need for a separate Nginx server on Render.
